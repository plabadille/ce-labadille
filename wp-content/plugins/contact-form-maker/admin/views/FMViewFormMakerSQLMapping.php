...
class FMViewFormMakerSQLMapping_fmc {
  ...
  ////////////////////////////////////////////////////////////////////////////////////////
    ...
    ?>
		<link media="all" type="text/css" href="<?php echo WD_FMC_URL . '/css/style.css'; ?>" rel="stylesheet">
		<link rel="stylesheet" href="<?php echo WD_FMC_URL . '/css/jquery-ui-1.10.3.custom.css'; ?>">
		<?php
		...
		function save_query() {
			...
			if(jQuery('#query_txt').val()) {
				...
				jQuery.ajax({
					type: "POST",
					url: "<?php echo add_query_arg(array('action' => 'FormMakerSQLMapping_fmc', 'id' => $id, 'form_id' => $form_id, 'task' => 'update_query', 'width' => '1000', 'height' => '500', 'TB_iframe' => '1'), admin_url('admin-ajax.php')); ?>",
					data: datatxt,
							...
							<div class="cols">
								...
								if($con_method=='insert' or $con_method=='update') {
									...
									foreach($table_struct as $key =>$col) {
										...
										$title.="</ul>";
										?>
										<div><label title="<?php echo $title; ?>" class="hasTip"><b><?php echo $col->Field; ?></b><img src="<?php echo WD_FMC_URL . '/images/info.png?ver='. WD_FMC_VERSION.''; ?>" style="width:20px; vertical-align:middle;padding-left: 10px;" /></label><input type="text" id="<?php echo str_replace("'", "SingleQuot", $col->Field); ?>" <?php if(!$col_checks[$key]) echo 'disabled="disabled"'?>  value="<?php echo $col_vals[$key]; ?>" /><input id="ch_<?php echo str_replace("'", "SingleQuot", $col->Field); ?>"  type="checkbox" onClick="dis('<?php echo str_replace("'", "SingleQuot", $col->Field); ?>', this.checked)" <?php if($col_checks[$key]) echo 'checked="checked"'?> /></div>
										<?php
									}
								}
								
								if($con_method=='delete' or $con_method=='update') {
									echo '<div class="select-db-op where"><img src="'.WD_FMC_URL . '/images/add_condition.png?ver='. WD_FMC_VERSION.'" title="ADD" class="add_cond"/>WHERE</div>';
									if($wheres) {
										echo '<script>';
	...
	public function add_query($form_id){
		...
		function connect() {
			...
			jQuery.ajax({
				type: "POST",
				url: "<?php echo add_query_arg(array('action' => 'FormMakerSQLMapping_fmc', 'id' => 0, 'form_id' => $form_id, 'task' => 'db_tables', 'width' => '1000', 'height' => '500', 'TB_iframe' => '1'), admin_url('admin-ajax.php')); ?>",
	...
	public function db_tables($form_id){
		...
		jQuery("#tables").change(function (){
			...
			jQuery.ajax({
				type: "POST",
				url: "<?php echo add_query_arg(array('action' => 'FormMakerSQLMapping_fmc', 'id' => 0, 'form_id' => $form_id, 'task' => 'db_table_struct', 'width' => '1000', 'height' => '500', 'TB_iframe' => '1'), admin_url('admin-ajax.php')); ?>",
    ...
    <script>
		...
		function save_query() {
			...
			if(jQuery('#query_txt').val()) {
				...
				jQuery.ajax({
					type: "POST",
					url: "<?php echo add_query_arg(array('action' => 'FormMakerSQLMapping_fmc', 'id' => 0, 'form_id' => $form_id, 'task' => 'save_query', 'width' => '1000', 'height' => '500', 'TB_iframe' => '1'), admin_url('admin-ajax.php')); ?>",
					data: datatxt,
					success: function(data) {
						window.parent.wd_fm_apply_options('apply_options');
						window.parent.tb_remove(); 
