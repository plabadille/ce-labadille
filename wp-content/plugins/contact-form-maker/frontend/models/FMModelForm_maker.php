...
class FMModelForm_maker_fmc {
  ...
  public function showform($id, $type = 'embedded') {
    global $wpdb;
    $row = $wpdb->get_row($wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'formmaker WHERE id="%d" AND id IN (' . get_option('contact_form_forms', 0) . ')', $id));
    ...
    $cssver = isset($form_theme['version']) ? $form_theme['version'] : rand();
		...
		wp_register_style('fm-style-' . $theme_id, WD_FMC_URL . '/css/frontend/fm-style-' . $theme_id . '.css', array(), $cssver);
  ...
  public function create_css($theme_id, $form_theme, $old = true, $force_rewrite = false) {
		$frontend_css = WD_FMC_DIR . '/css/frontend/fm-style-' . $theme_id . '.css';
...
.fm-form-container.fm-theme" . $theme_id . " .minimize-form {
	top: ".$form_theme['CBPTop'].";
	".$for_mini.": ".(2 * (int)$form_theme['CBP'.ucfirst($for_mini)] + (int)$form_theme['CBPFontSize'] + 3)."px;
	bottom: ".$form_theme['CBPBottom'].";
}";
...
.fm-form-container.fm-theme" . $theme_id . " .fm-form .file-picker{
	background: url(".WD_FMC_URL.'/'.$form_theme['OPFBgUrl'].") ".$form_theme['OPFBGRepeat']." ".$form_theme['OPFPos1']." ".$form_theme['OPFPos2'].";
	display: inline-block;
}

.fm-form-container.fm-theme" . $theme_id . " .fm-form .wdform-calendar-button {
	background: url(".WD_FMC_URL.'/'.$form_theme['OPDPIcon'].") ".$form_theme['OPDPRepeat']." ".$form_theme['OPDPPos1']." ".$form_theme['OPDPPos2']."; 
...
.fm-form-container.fm-theme" . $theme_id . " .fm-form select {
	...
	background:".$form_theme['IPBGColor']." url(".WD_FMC_URL.'/'.$form_theme['SBPBackground'].") ".$form_theme['SBPBGRepeat']." ".$form_theme['SBPBGPos1']." ".$form_theme['SBPBGPos2'].";
...
.fm-form-container.fm-theme" . $theme_id . " .fm-form .checkbox-div input[type='checkbox']:checked + label span:after {
	...
	background:".($form_theme['MCCPBackground'] ? ($form_theme['MCCPBGColor']." url(".WD_FMC_URL.'/'.$form_theme['MCCPBackground'].") ".$form_theme['MCCPBGRepeat']." ".$form_theme['MCCPBGPos1']." ".$form_theme['MCCPBGPos2']) : $form_theme['MCCPBGColor']).";
...
.fm-form-container.fm-theme" . $theme_id . " .fm-form input[type='radio'] {
    ...
    if($form_theme['CUPCSS']) {
      ...
      $css_content .=  str_replace('[SITE_ROOT]', WD_FMC_URL, $theme);
    }
    ...
    if ($form->paypal_mode) {
      if ($paypal['item_name']) {
        if ($is_amount) {
          ...
          $str .= "&cmd=" . "_cart";
          $str .= "&notify_url=" . admin_url('admin-ajax.php?action=checkpaypal_fmc%26form_id=' . $id . '%26group_id=' . ($max + 1));
          $str .= "&upload=" . "1";
          $str .= "&charset=UTF-8";
          if (isset($paypal['shipping'])) {
