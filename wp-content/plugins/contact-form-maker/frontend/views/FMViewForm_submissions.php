...
class FMViewForm_submissions_fmc {
  ////////////////////////////////////////////////////////////////////////////////////////
  // Events                                                                             //
  ////////////////////////////////////////////////////////////////////////////////////////
  ////////////////////////////////////////////////////////////////////////////////////////
  ...
  public function display($form_id, $startdate, $enddate, $submit_date, $submitter_ip, $username, $useremail, $form_fields, $csv, $xml, $title, $search, $ordering, $entries, $views, $conversion_rate, $pagination, $stats) {
    ...
    if ($get_data === false) {
      echo WDW_FMC_Library::message(__('You have no permission to view submissions.', 'form_maker'), 'warning');
      return;
    }
    $rows	= $get_data["rows"];
    ...
    <form action="<?php echo $current_url; ?>" method="post" name="adminForm" id="adminForm">
      ...
      <div>
        ...
        <?php endif; if($csv || $xml):?>
          ...
          <?php if($csv): ?>
          <input type="button" value="CSV" onclick="window.location='<?php echo add_query_arg(array('action' => 'frontend_generate_csv_fmc', 'page' => 'form_submissions', 'id' => $form_id, 'checked_ids' => $export_ids, 'from' => $startdate, 'to' => $enddate), admin_url('admin-ajax.php')) ?>'" />&nbsp;
          <?php endif; ?>
          <?php if($xml): ?>
          <input type="button" value="XML" onclick="window.location='<?php echo add_query_arg(array('action' => 'frontend_generate_xml_fmc', 'page' => 'form_submissions', 'id' => $form_id, 'checked_ids' => $export_ids, 'from' => $startdate, 'to' => $enddate), admin_url('admin-ajax.php')) ?>'" />&nbsp;
          <?php endif; ?>
          </div>
        <?php endif; ?>	
      </div>
      ...
      <div class="search_and_pagination">
        ...
        <div>
          ...
          <img id="filter_img" src="<?php echo WD_FMC_URL . '/images/filter_show.png'; ?>" width="40" style="vertical-align:middle; cursor:pointer" onclick="show_hide_filter()"  title="Search by fields" />
        ...
        <div class="tablenav top">
          <?php WDW_FMC_Library::html_page_nav($lists['total'], $lists['limit'], 'adminForm'); ?>
        </div>  
        <?php endif; ?>
      </div>
      <?php endif; ?>
        ...
        for($www=0, $qqq=count($group_id_s); $www < $qqq ; $www++) {
          ...
          <tr class="<?php echo "row$k"; ?>">
                ...
                for($g=0; $g < $ttt ; $g++) {
                  ...
                  if($t->element_label==$labels_id[$h]) {
                    if(strpos($t->element_value,"***map***")) {
                      ...
                      $longit	= $map_params[0];
                      $latit = $map_params[1];                      
                      echo  '<td align="center" class="'.$labels_id[$h].'_fc" '.$styleStr.'><a class="thickbox-preview" href="' . add_query_arg(array('action' => 'frontend_show_map_fmc', 'page' => 'form_submissions', 'long' => $longit, 'lat' => $latit, 'width' => '620', 'height' => '550', 'TB_iframe' => '1'), admin_url('admin-ajax.php')) . '" title="' . __('Show on Map', 'form_maker') . '">' . __('Show on Map', 'form_maker') . '</a></td>';
                    }
                    else {
                        ...
                        if(strpos($t->element_value,"@@@")>-1 || $t->element_value=="@@@" || $t->element_value=="@@@@@@@@@" || $t->element_value=="::" || $t->element_value==":" || $t->element_value=="--") {
                          ...
                          if(strpos($t->element_value,"***grading***")) {
                            ...
                            if(strpos($t->element_value,"***matrix***")) {
                              echo  '<td align="center" class="'.$labels_id[$h].'_fc" '.$styleStr.'><a class="thickbox-preview" href="' . add_query_arg(array('action' => 'frontend_show_matrix_fmc', 'page' => 'form_submissions', 'matrix_params' => $t->element_value, 'width' => '620', 'height' => '550', 'TB_iframe' => '1'), admin_url('admin-ajax.php')) . '" title="' . __('Show Matrix', 'form_maker') . '">' . __('Show Matrix', 'form_maker') . '</a></td>';
                            }
                            else {
            ...
            if($form_fields && $payment_info) {
              ...
              <a class="thickbox-preview" href="' . add_query_arg(array('action' => 'frontend_paypal_info_fmc', 'page' => 'form_submissions', 'id' => $i, 'width' => '600', 'height' => '500', 'TB_iframe' => '1'), admin_url('admin-ajax.php')) . '">
                  <img src="' . WD_FMC_URL . '/images/info.png" />
                </a></td>';
            }
            ?>
          </tr>
    ...
    <script> 
      ...
      function show_stats() {
        ...
        if(jQuery('#stat_id').val() != "") {
          jQuery('#div_stats').load('<?php echo add_query_arg(array('action' => 'get_frontend_stats_fmc', 'page' => 'form_submissions'), admin_url('admin-ajax.php')); ?>', {
